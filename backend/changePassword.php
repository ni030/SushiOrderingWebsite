...
if (isset($_POST["MM_Update"]) && $_POST["MM_Update"] == "Confirm") {
    if (empty($_POST["currentPW"]) || empty($_POST["newPW"])) {
        ...
        if ($decryption !== $currentPW) {
            header("Location: ../frontend/changePass.php?change=current");
            exit();
        } else {
            // Password Encryption
            // Store the cipher method
            ...
            if (!$resultUpdate) {
                header("Location: ../frontend/changePass.php?change=error");
                exit();
            } else {
                header("Location: ../frontend/changePass.php?change=success");
            }
        }
    }
}
